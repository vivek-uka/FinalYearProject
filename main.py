...
class ModelPredictiveControl:
	...
	def cost_function(self, state, u): 
		...
		psi = state[2] + np.array([(i + 1) * u[2*i + 1] * self.dt for i in range(self.horizon)])
		rn = state[0] + np.array([u[2*i + 0] * np.cos(psi) * self.dt for i in range(self.horizon)])
		re = state[1] + np.array([u[2*i + 0] * np.sin(psi) * self.dt for i in range(self.horizon)])
...
if __name__ == '__main__':
	...
	myRobot = ModelPredictiveControl(5.0, 5.0, 2.84, 0.22)
